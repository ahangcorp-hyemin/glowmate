...
- **채움률 실측치:** 좌표 96~99% · 도로명주소 100% · 전화는 **축마다 크게 다르다** —
  `medical_wellness` **85.1%** · `exercise_body` **30.9%** · `beauty_care` **21%**.
  `v1.2` 까지 "의료·체육 81~82%"로 적혀 있었으나, 이는 **의료 쪽 수치를 체육에까지 확장 기재한
  오류**였다(구현 시 두 취득 경로가 동일하게 407/1,316 = 0.3093 으로 실측 확인). **결측은 결측으로 남긴다.**
...
requirements:
  ...
  - id: REQ-6
    ...
    acceptance: >
      ...
      ④ `phone` 채움률이 `beauty_care` 축 ∈ [0.10, 0.40], `exercise_body` 축 ∈ [0.25, 0.45],
      `medical_wellness`·`relax_recovery` 축 ∈ [0.60, 0.95] ·
